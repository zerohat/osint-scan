...
if [ "${EYE_WITNESS}" == "YES" ]; then
  ...
  if [ -f "${BASE}/.tmp-webserver-port81" ]; then
   rm -f ${BASE}/.tmp-webserver-port81
  fi

  egrep "81/open" $(ls -1 ${BASE}/scan-${CUSTOMER}*.gnmap | tail -1) |awk '{print $2,$3}' >>${BASE}/.tmp-webserver-port81
  if [ -s "${BASE}/.tmp-webserver-port81" ]; then
   cat ${BASE}/.tmp-webserver-port81 |cut -d' ' -f1 |sed 's/^/http:\/\//' | sed 's/$/:81/' >${BASE}/urlliste-port81
   cat ${BASE}/.tmp-webserver-port81 |cut -d' ' -f2 |sed -e 's/^(//g' | sed -e 's/)//g' |sed '/^$/d' |sed 's/^/http:\/\//' | sed 's/$/:81/' >>${BASE}/urlliste-port81
  fi
